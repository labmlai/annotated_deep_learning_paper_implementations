...
---
title: Style GAN2
summary: A minimal PyTorch implementation/tutorial of Style GAN2.
---

# Style GAN2

This is a [PyTorch](https://pytorch.org) implementation of the paper
[Analyzing and Improving the Image Quality of StyleGAN](https://arxiv.org/abs/1912.04958) which
introduces **Style GAN2**.
Style GAN2 is an improvement over **Style GAN** introduced in the paper
...
*Our implementation is a stand-alone minimalistic single GPU Style GAN2 training code.
We've tried to keep it simple and understandable, and we managed to shrink it to a little more
than 350 lines of code.*

Generative adversarial networks have two components; the generator and the discriminator.
The generator network takes a random latent vector and tries to generate a real looking image.
The discriminator network tries to differentiate the real images from generated images.
When we train the two together the generator starts generating images indistinguishable from real images.

## Progressive GAN

Progressive GAN trains neural networks with adversarial training to generate high-resolution
images of size 1028. It does so by *progressively* increasing the image size. First,
it trains a network that produces a 4x4 image, then an 8x8 image, then an 16x16 image, and so on.

At each resolution the generator network produces an image in latent space which is converted into RGB,
with a 1x1 convolution. When we progress from a lower resolution to a higher resolution (say from 16x16 to 32x32)
we scale the latent image by 2X and add a new block (two 3x3 convolution layers) and a new 1x1 layer to get RGB.
The transition is done smoothly by adding a residual connection to the 2X scaled 16x16 RGB image.
...
They use something called equalized learning rates which is discussed below in the implementation.
...
First it maps the random latent vector ($z \in \mathcal{Z}$)
into a different latent space ($w \in \mathcal{W}$), with a 8-layer neural network.
"""
